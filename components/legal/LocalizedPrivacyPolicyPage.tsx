...
import { LEGAL_TERMS_PATHNAME } from "@/lib/legal-routes";
...
export async function LocalizedPrivacyPolicyPage() {
  const locale = await getLocale();
  const t = await getTranslations("legal");
  const sections = t.raw("privacyPage.sections") as Record<string, SectionBlock>;
  ...
  return (
    <PublicLegalPageShell
      ...
      footer={
        <p>
          {t("privacyPage.footerBefore")}
          <Link href={LEGAL_TERMS_PATHNAME} className="text-primary hover:underline">
            {t("privacyPage.footerLink")}
          </Link>
          {t("privacyPage.footerAfter")}
        </p>
      }
    >
      {renderSection(sections.whoWeAre)}
      {renderSection(sections.dataCollected)}
      {renderSection(sections.purposes)}
      {renderSection(sections.storage)}
      <LegalSection title={sections.rights.title}>
        <p>{sections.rights.intro}</p>
        {renderList(sections.rights.list)}
      </LegalSection>
      {renderSection(sections.cookies)}
