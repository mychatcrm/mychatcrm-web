  ...
  return (
    <section className={cn(hubGlass, "p-5 sm:p-6")}>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">OpenAI Platform</p>
          <h2 className="mt-1 text-lg font-semibold text-content">Conta oficial · billing</h2>
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="secondary" type="button" className="border-line bg-surface-card text-content" onClick={onToggleLiveSync}>
            {liveSync ? "Pausar sync" : "Retomar sync"}
          </Button>
          <Button variant="secondary" type="button" className="border-line bg-surface-card text-content" disabled={openAiLoading} onClick={onRefreshOpenAi}>
      ...
      {!openAi && openAiLoading ? <div className="h-20 animate-pulse rounded-xl bg-surface-elevated" /> : null}
      ...
      {openAi?.configured && showBilling403Info ? (
        <div className="mb-3 rounded-xl border border-primary/25 bg-primary/10 p-3 text-xs text-content-secondary">
      ...
      {endpointRows.length > 0 ? (
        <details className="mt-4 rounded-xl border border-line bg-surface-elevated p-3 text-xs text-content-muted">
          <summary className="cursor-pointer font-medium text-content">Diagnóstico HTTP</summary>
...
function Metric({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className="rounded-xl border border-line bg-surface-elevated p-3">
      ...
      {sub ? <p className="mt-0.5 text-[10px] text-content-muted">{sub}</p> : null}
    </div>
  );
}
