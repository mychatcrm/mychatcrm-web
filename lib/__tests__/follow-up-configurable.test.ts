...
// O horário efetivo do worker sempre vence o horário originalmente agendado.
// Um cron atrasado não recebe permissão para enviar fora da janela.
describe("follow-up atrasado continua preso à janela atual", () => {
  const base = { ...DEFAULT_FOLLOW_UP_INTELIGENTE, ativo: true, timezone: "UTC" };

  it("não envia no sábado só porque o job nasceu numa sexta permitida", () => {
    const ctx = makeCtx(base, {
      now: new Date("2026-05-23T04:00:00.000Z"),
      job: {
        id: "j",
        attempts: 0,
        maxAttempts: 3,
  ...
  it("reagenda sem consumir tentativa", () => {
    const ctx = makeCtx(base, {
    ...
    expect(d.shouldSend).toBe(false);
    expect(d.skipReason).toBe("outside_business_hours");
    expect(ctx.job.attempts).toBe(5);
    expect(d.nextRetryAt?.getTime()).toBeGreaterThan(ctx.now.getTime());
  });
});
