import { getPlanPolicy, type PlanLimits } from "@/lib/plan-policy";
...
export function enterpriseLimitsToPlanLimits(limits: EnterpriseProvisionLimits): PlanLimits {
  ...
  return {
    ...
    // Não é campo do provisionamento: o admin não configura agente de disparos
    // por contrato enterprise. Segue a política do plano (1 para todos) — se um
    // dia virar negociável, entra como campo próprio em vez de virar número
    // solto aqui.
    includedBroadcastAgents: getPlanPolicy("enterprise").includedBroadcastAgents,
