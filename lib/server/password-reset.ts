 ...
 *
 * Admin e clientes usam tokens de uso único enviados por e-mail. A senha só é
 * alterada quando o utilizador abre o link e define uma nova palavra-passe.
 *
...
export async function requestPasswordReset(params: {
  ...
  if (isResendTestSender()) {
    console.error(
      "[password-reset] reset link blocked: Resend sender is still using @resend.dev test mode. Verify a domain in Resend and set RESEND_FROM_EMAIL before creating password reset tokens.",
      "scope:",
      params.scope,
    );
    if (isForgotPasswordForensicEnabled()) {
      console.warn(
        ...
        JSON.stringify({ stage: "reset_link_test_sender_blocked", scope: params.scope }),
      );
    }
    return { sent: false, mailConfigured };
  }
