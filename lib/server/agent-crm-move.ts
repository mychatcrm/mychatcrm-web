...
export type AgentCrmMoveAction = "scheduled" | "rescheduled" | "cancelled" | "lead_replied";
...
/**
 * Move o card na PRIMEIRA resposta do lead e só nela.
 *
 * O carimbo `leads.first_reply_at` é a trava: reivindicá-lo é uma escrita
 * condicional (`.is("first_reply_at", null)`), então mesmo com duas mensagens
 * chegando ao mesmo tempo só uma ganha e o card é movido uma vez. Depois disso
 * o lead pode responder à vontade que o card fica onde a equipe deixou.
 */
export async function applyCrmMoveOnFirstLeadReply(params: {
  sb: SupabaseServiceClient;
  tenantId: string;
  agentId?: string | null;
  leadId?: string | null;
  ...
  try {
    ...
    const claimed = await claimFirstReply(params.sb, params.tenantId, leadId);
    ...
    console.warn("[agent-crm-move] first_reply_unexpected_error", {
      tenant_id: params.tenantId,
      error: err instanceof Error ? err.message : String(err),
    });
    return "failed";
...
/**
 * Carimba `first_reply_at` só se ainda estiver vazio. Devolve true quando ESTA
 * chamada foi a que carimbou — ou seja, é a primeira resposta.
 *
 * Base sem a coluna (instalação anterior à migration) devolve false: sem a
 * trava não dá pra garantir "uma vez só", e mover repetidamente seria pior do
 * que não mover.
 */
async function claimFirstReply(
  sb: SupabaseServiceClient,
  tenantId: string,
  leadId: string,
): Promise<boolean> {
  ...
  if (error) {
    if (!isMissingColumn(error, "first_reply_at")) {
      console.warn("[agent-crm-move] first_reply_claim_failed", {
        tenant_id: tenantId,
        lead_id: leadId,
        code: error.code,
        error: error.message,
