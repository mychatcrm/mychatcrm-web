...
import { listInvoicesAdmin } from "@/lib/server/admin-stripe-metrics";

export const dynamic = "force-dynamic";

export async function GET(request: Request) {
  const auth = await enforceAdminFinanceStripe("faturas");
  ...
  try {
    const payload = await listInvoicesAdmin({
      fromSec: q.fromSec,
      toSec: q.toSec,
      limit: q.limit,
      cursor: q.cursor,
      status: url.searchParams.get("invoiceStatus"),
      planSlug: q.planSlug,
    });
    ...
    console.error("[admin/stripe/faturas]", msg);
