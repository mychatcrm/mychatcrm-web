  ...
  try {
    ...
    if (tenantId) {
      const [a,e,c,r] = await Promise.all([
        ...
        sb.from("whatsapp_cloud_connections").select("phone_number_id,display_phone,active,slot_index").eq("tenant_id", tenantId).limit(100),
      ...
      connections = [...(e.data ?? []).map(row => ({ id: row.id, channel: "evolution", slot: row.slot_index, state: row.connection_state, number: labMaskedJid(labPhoneJid(row.wa_jid)) })),
        ...(c.data ?? []).map(row => ({ id: row.phone_number_id, channel: "meta_cloud", slot: row.slot_index, state: row.active ? "open" : "close", number: labMaskedJid(labPhoneJid(row.display_phone)) }))];
    }
