...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Erro ao listar mídia.";
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json(
      { file: toApiFile(result.file), uploadUrl: result.uploadUrl, expiresInSeconds: result.expiresInSeconds },
      { status: 201 },
    );
  } catch (error) {
