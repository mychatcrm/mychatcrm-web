...
export function PlansComparisonTable() {
  return (
    ...
    >
      ...
      <div className="mt-6 overflow-x-auto rounded-xl border border-line/80 bg-surface-card">
        <table className="w-full min-w-[480px] border-collapse text-left text-[11px] text-content sm:min-w-[520px] sm:text-xs">
          <thead>
            <tr className="border-b border-line bg-surface-deep/50">
              <th
                scope="col"
                className="sticky left-0 z-20 w-[min(42%,11rem)] max-w-[11rem] border-r border-line/50 bg-surface-deep px-2 py-2 text-[9px] font-semibold uppercase leading-tight tracking-wide text-content-muted sm:w-auto sm:max-w-[13rem] sm:px-2.5 sm:py-2 sm:text-[10px]"
              >
                Funcionalidade
              </th>
              ...
              <Fragment key={section.category}>
                ...
                {section.rows.map((row) => (
                  <tr key={row.label} className="transition hover:bg-surface-deep/[0.12]">
                    <th
                      scope="row"
                      className="sticky left-0 z-10 max-w-[11rem] border-r border-line/40 bg-surface-card px-2 py-1.5 text-left text-[10px] font-medium leading-snug text-content sm:max-w-[13rem] sm:px-2.5 sm:text-[11px]"
                    >
                      {row.label}
                    </th>
                    {SALES_PLANS_COMPARISON_COLUMNS.map((plan) => (
